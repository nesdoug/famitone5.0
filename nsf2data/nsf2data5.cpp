// nsf2data5.cpp : Defines the entry point for the console application.
// edited to allow effects over 256 bytes. per rainwarrior's suggestions.
...
unsigned char effect_data[MAX_SIZE];
...
{
	if (effect_ptr<MAX_SIZE) effect_data[effect_ptr] = n;
...
void convert_effects(void)
{
	...
	for (song = 0; song<nsf_songs; ++song)
	{
		...
		for (mode = 0; mode<2; ++mode)
		{
			...
			if (effect_ptr>MAX_SIZE)
			{
				printf("\nError: effect data is too long, should be 10000 bytes max.\n");
				effect_error = 1;
			}
